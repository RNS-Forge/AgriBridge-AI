...
async function runTests() {
    ...
    try {
        ...
        const getJson = await getFarmerRes.json();
        ...
        if (getFarmerRes.status !== 200)
            throw new Error('Get Farmer details failed');
        ...
        if (listRes.status !== 200)
            throw new Error('List farmers failed');
        ...
        if (updateRes.status !== 200)
            throw new Error('Update farmer failed');
        ...
        if (deleteRes.status !== 200)
            throw new Error('Delete farmer failed');
