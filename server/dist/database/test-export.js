...
import { users, tenants, crops, pools, buyers, orders, marketplaceListings, marketplaceOffers, batches, harvests, farms, farmers, exports as exportTable, shipments, certificates, auditLogs, userRoles, roles } from '../db/schema.js';
...
async function runTests() {
    ...
    try {
        ...
        if (check1Json.data.eligible === true)
        ...
        if (check2Json.data.eligible !== true)
            throw new Error('Eligibility check should have passed');
        ...
        if (updateJson.data.customsStatus !== 'approved')
            throw new Error('Customs status update failed');
        ...
        if (shipRes.status !== 201)
            throw new Error('Shipment registration failed');
        // Database Teardown
