...
async function runTests() {
    ...
    try {
        ...
        const farmJson = await regFarmRes.json();
        console.log('Register Farm Status:', regFarmRes.status);
        console.log('Register Farm Body:', farmJson);
        if (regFarmRes.status !== 201)
            throw new Error('Farm registration failed');
        ...
        const getJson = await getFarmRes.json();
        ...
        if (getFarmRes.status !== 200)
            throw new Error('Get Farm details failed');
        ...
        if (updateRes.status !== 200)
            throw new Error('Update farm failed');
        console.log('\n--- Step 9: Delete Farm ---');
        ...
        if (deleteRes.status !== 200)
            throw new Error('Delete farm failed');
        // Database Teardown
