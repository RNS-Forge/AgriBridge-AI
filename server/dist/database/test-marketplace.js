...
import { users, tenants, crops, pools, buyers, orders, marketplaceListings, marketplaceOffers, marketplaceWishlists, auditLogs, userRoles, roles } from '../db/schema.js';
...
async function runTests() {
    ...
    try {
        ...
        if (searchRes.status !== 200 || searchJson.data.length === 0)
        ...
        const getWishJson = await getWishRes.json();
        ...
        if (offerRes.status !== 201)
            throw new Error('Offer submit failed');
        ...
        if (ordersResult.length === 0)
            throw new Error('PO was not generated');
        // Database Teardown
