...
async function runTests() {
    ...
    try {
        ...
        const harvestJson = await regHarvestRes.json();
        console.log('Register Harvest Status:', regHarvestRes.status);
        console.log('Register Harvest Body:', harvestJson);
        if (regHarvestRes.status !== 201)
            throw new Error('Harvest registration failed');
        ...
        if (traceRes.status !== 200)
            throw new Error('Trace report failed');
        ...
        if (checkHarvest[0]?.status !== 'unbatched')
            throw new Error('Harvest status was not reverted correctly');
        // Database Teardown
