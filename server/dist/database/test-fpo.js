...
import { users, tenants, farmers, farms, crops, harvests, batches, pools, fpoShares, fpoProfitSplits, fpoProfitAllocations, fpoInvitations, auditLogs, userRoles, roles } from '../db/schema.js';
...
async function runTests() {
    ...
    try {
        ...
        if (bulkRes.status !== 200)
            throw new Error('Bulk upload request failed');
        console.log('Success Count:', bulkJson.data.successCount);
        console.log('Failed Count:', bulkJson.data.failedCount);
        if (bulkJson.data.successCount !== 3)
            throw new Error('Bulk upload did not register all members');
        uploadedFarmers = bulkJson.data.members.map((m) => m.farmerId);
        ...
        for (const conf of sharesConfigs) {
            ...
            if (shareRes.status !== 200)
                throw new Error(`Shares allocation failed for farmer ${conf.farmerId}`);
        }
        ...
        const cropJson = await regCropRes.json();
        ...
        for (const conf of harvestConfigs) {
            ...
            const harvJson = await harvRes.json();
        ...
        const splitSharesJson = await splitSharesRes.json();
        ...
        const detailSharesJson = await detailSharesRes.json();
        console.log('Shares Dist Payout Allocations:');
        detailSharesJson.data.allocations.forEach((a) => {
        ...
        const splitPoolJson = await splitPoolRes.json();
        ...
        const detailPoolJson = await detailPoolRes.json();
        console.log('Pool Dist Payout Allocations:');
        detailPoolJson.data.allocations.forEach((a) => {
            console.log(`- ${a.farmerName}: Payout: ${a.payoutAmount} INR (Status: ${a.status})`);
        });
        // Database Teardown
