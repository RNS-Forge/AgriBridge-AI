...
import { users, tenants, auditLogs, userRoles, roles, platformSettings } from '../db/schema.js';
...
async function runTests() {
    ...
    try {
        ...
        if (!superAdminRole[0])
            throw new Error('SuperAdmin role missing from DB');
        ...
        const listTenantsJson = await listTenantsRes.json();
        ...
        if (upgradeJson.data.subscriptionPlan !== 'enterprise')
        ...
        if (suspendJson.data.status !== 'suspended')
            throw new Error('Tenant suspension failed');
        ...
        if (auditRes.status !== 200)
            throw new Error('Audit logs query failed');
        // Database Teardown
