...
async function runTests() {
    ...
    try {
        ...
        const farmerJson = await regFarmerRes.json();
        console.log('Register Farmer Status:', regFarmerRes.status);
        if (regFarmerRes.status !== 201)
        ...
        if (regCropRes.status !== 201)
            throw new Error('Crop master registration failed');
        ...
        const mapJson = await regMapRes.json();
        console.log('Map Farmer Crop Status:', regMapRes.status);
        console.log('Map Farmer Crop Body:', mapJson);
        if (regMapRes.status !== 201)
            throw new Error('Farmer crop mapping failed');
        ...
        const getJson = await getMappingRes.json();
        ...
        if (getMappingRes.status !== 200)
            throw new Error('Get mapping failed');
        ...
        if (updateRes.status !== 200)
            throw new Error('Update mapping failed');
        ...
        if (invalidRes.status !== 500)
            throw new Error('Should have failed date validation check');
        ...
        if (delMapRes.status !== 200)
            throw new Error('Delete mapping failed');
        ...
        if (delCropRes.status !== 200)
            throw new Error('Delete crop failed');
        // Database Teardown
        ...
        const userIds = [];
        if (adminUser[0])
            userIds.push(adminUser[0].id);
        if (farmerUser[0])
            userIds.push(farmerUser[0].id);
        if (userIds.length > 0) {
