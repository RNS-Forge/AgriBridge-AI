export function tenantAware(req, res, next) {
    const authReq = req;
    // Try to get tenantId from headers, query parameters, or route parameters
    const headerTenantId = authReq.headers['x-tenant-id'];
    const queryTenantId = authReq.query.tenantId;
    const paramTenantId = authReq.params.tenantId;
