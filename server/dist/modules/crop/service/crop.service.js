...
export class CropService {
    ...
    // ==========================================
    async createCrop(dto, actorUserId) {
        const data = {
            name: dto.name,
    ...
    async updateCrop(cropId, dto, actorUserId) {
        ...
        if (dto.variety !== undefined)
            data.variety = dto.variety;
        if (dto.scientificName !== undefined)
            data.scientificName = dto.scientificName;
        if (dto.hsCode !== undefined)
            data.hsCode = dto.hsCode;
    ...
    async deleteCrop(cropId, actorUserId) {
        const existing = await this.cropRepository.findCropById(cropId);
        if (!existing) {
            throw new Error('Crop not found');
        }
    ...
    // ==========================================
    async mapFarmerCrop(dto, actorUserId) {
        let sowing = null;
        let harvest = null;
        if (dto.sowingDate) {
    ...
    async updateFarmerCrop(mappingId, dto, actorUserId, actorTenantId) {
        ...
        if (dto.sowingDate !== undefined)
            updateData.sowingDate = dto.sowingDate ? new Date(dto.sowingDate) : null;
        if (dto.expectedHarvestDate !== undefined)
            updateData.expectedHarvestDate = dto.expectedHarvestDate ? new Date(dto.expectedHarvestDate) : null;
        if (dto.expectedYieldKg !== undefined)
            updateData.expectedYieldKg = dto.expectedYieldKg.toString();
    ...
    async deleteFarmerCrop(mappingId, actorUserId, actorTenantId) {
        const existing = await this.cropRepository.findFarmerCropById(mappingId);
        if (!existing) {
            throw new Error('Farmer crop mapping not found');
        }
