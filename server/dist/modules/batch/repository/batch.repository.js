...
import { harvests, batches, farms, farmers, crops, users, qualityInspections, orders, exports as exportTable, shipments, auditLogs } from '../../../db/schema.js';
...
export class BatchRepository {
    ...
    async findBatchByTraceabilityCode(code) {
        const result = await db
            .select()
            .from(batches)
    ...
    async listBatchesByTenant(tenantId) {
        return await db
            .select()
            .from(batches)
    ...
    // ==========================================
    async getTraceabilityInfo(batchId) {
        ...
        const history = await db
            ...
            .where(and(eq(auditLogs.entityName, 'batches'), eq(auditLogs.entityId, batchId)));
        return {
            ...info,
