...
export class MarketplaceService {
    ...
    // LISTINGS
    // ==========================================
    async createListing(dto, actorUserId) {
        const listingData = {
    ...
    // ==========================================
    async submitOffer(dto, actorUserId) {
    ...
    async fpoCounterOffer(offerId, counterPricePerKg, actorUserId, actorTenantId) {
        ...
        const updated = await this.marketplaceRepository.updateOfferStatus(offerId, 'countered', counterPricePerKg.toString(), 'fpo');
    ...
    async acceptOffer(offerId, actorUserId, actorTenantId) {
        const existing = await this.marketplaceRepository.findOfferById(offerId);
        if (!existing) {
            throw new Error('Offer not found');
        }
        ...
        const finalPrice = parseFloat(existing.offer.status === 'countered' && existing.offer.offeredBy === 'fpo'
    ...
    async rejectOffer(offerId, actorUserId) {
        const existing = await this.marketplaceRepository.findOfferById(offerId);
        if (!existing) {
            throw new Error('Offer not found');
        }
    ...
    // WISHLIST
    // ==========================================
    async addToWishlist(buyerId, listingId) {
