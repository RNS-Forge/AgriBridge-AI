...
export class ExportService {
    ...
    formatCertificateResponse(cert) {
        return {
            id: cert.id,
    ...
    // ==========================================
    async issueCertificate(dto, actorUserId) {
        const certData = {
            batchId: dto.batchId,
    ...
    // EXPORTS
    // ==========================================
    async initiateExport(dto, actorUserId) {
        const exportData = {
            orderId: dto.orderId,
    ...
    async updateExport(exportId, dto, actorUserId) {
        ...
        if (dto.portOfLoading !== undefined)
            data.portOfLoading = dto.portOfLoading;
        if (dto.portOfDischarge !== undefined)
            data.portOfDischarge = dto.portOfDischarge;
        if (dto.customsStatus !== undefined)
            data.customsStatus = dto.customsStatus;
        if (dto.customsDeclarationNumber !== undefined)
            data.customsDeclarationNumber = dto.customsDeclarationNumber;
        if (dto.commercialInvoiceNumber !== undefined)
            data.commercialInvoiceNumber = dto.commercialInvoiceNumber;
        if (dto.commercialInvoiceUrl !== undefined)
            data.commercialInvoiceUrl = dto.commercialInvoiceUrl;
        if (dto.packingListUrl !== undefined)
            data.packingListUrl = dto.packingListUrl;
    ...
    // ==========================================
    async checkEligibility(exportId, actorUserId) {
        ...
        let matchingPhytoNum = null;
        let matchingOriginNum = null;
        if (ctx.batches.length > 0) {
            for (const batch of ctx.batches) {
                ...
                if (phytoCerts.length === 0)
                    phytoPassed = false;
                else
                    matchingPhytoNum = phytoCerts[0].certificateNumber;
                if (originCerts.length === 0)
                    originPassed = false;
                else
                    matchingOriginNum = originCerts[0].certificateNumber;
            }
        }
        else {
            phytoPassed = false;
    ...
    // LOGISTICS SHIPMENT
    // ==========================================
    async registerShipment(dto, actorUserId) {
    ...
    async getShipmentDetails(exportId) {
