...
export class FpoService {
    fpoRepository = new FpoRepository();
    formatInvitationResponse(inv) {
        return {
            id: inv.id,
    ...
    // INVITATIONS
    // ==========================================
    async createInvitation(dto, actorUserId) {
    ...
    // SHARES LEDGER
    // ==========================================
    async manageShares(dto, actorUserId) {
        const sharesData = {
            tenantId: dto.tenantId,
    ...
    // ==========================================
    async distributeProfit(dto, actorUserId) {
        ...
        if (dto.splitType === 'by_shares') {
            ...
            const totalShares = sharesList.reduce((sum, item) => sum + item.shares.sharesCount, 0);
            if (totalShares === 0) {
                throw new Error('Total shares count in FPO is zero.');
            }
            // 2. Proportional allocation by shares
            for (const item of sharesList) {
    ...
    async listProfitSplits(tenantId) {
    ...
    async getProfitSplitDetails(splitId) {
    ...
    // ==========================================
    async bulkUploadMembers(tenantId, members, actorUserId) {
        const responseList = [];
        let successCount = 0;
        let failedCount = 0;
