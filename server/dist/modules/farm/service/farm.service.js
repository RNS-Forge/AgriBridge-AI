...
export class FarmService {
     ...
     * Register a new farm.
     */
    async registerFarm(dto, actorUserId) {
     ...
     * Update farm details.
     */
    async updateFarm(farmId, dto, actorUserId, actorTenantId) {
        ...
        if (dto.soilType !== undefined)
            updateData.soilType = dto.soilType;
        if (dto.totalAreaHectares !== undefined)
            updateData.totalAreaHectares = dto.totalAreaHectares.toString();
        if (dto.latitude !== undefined)
            updateData.latitude = dto.latitude.toString();
        if (dto.longitude !== undefined)
            updateData.longitude = dto.longitude.toString();
        if (dto.surveyNumber !== undefined)
            updateData.surveyNumber = dto.surveyNumber;
        if (dto.boundaryCoordinates !== undefined)
            updateData.boundaryCoordinates = dto.boundaryCoordinates;
        if (dto.waterSource !== undefined)
            updateData.waterSource = dto.waterSource;
     ...
     * Delete farm.
     */
    async deleteFarm(farmId, actorUserId, actorTenantId) {
        const existing = await this.farmRepository.findFarmById(farmId);
        if (!existing) {
            throw new Error('Farm not found');
        }
