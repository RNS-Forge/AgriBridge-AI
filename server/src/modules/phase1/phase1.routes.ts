...
router.post('/auth/login-verify-2fa', (req: Request, res: Response) => {
...
router.post('/auth/user-requests/:id/approve', async (req: Request, res: Response) => {
  ...
  const newUser = {
    ...
    profilePicture: request.profilePicture || undefined,
...
router.post('/auth/user-requests/bulk-approve', async (req: Request, res: Response) => {
  const authUser = getAuthUser(req);
  const { ids } = req.body;
  ...
  for (const id of ids) {
    const request = phase1Store.userRequests.find((r) => r.id === id);
    if (!request || request.status !== 'PENDING_APPROVAL') continue;
...
router.post('/auth/user-requests/bulk-reject', (req: Request, res: Response) => {
  const authUser = getAuthUser(req);
...
// Admin Direct Create User Handler (Can create any role instantly, including Admin Maker and Admin)
const handleAdminCreateUser = async (req: Request, res: Response) => {
  const authUser = getAuthUser(req);
  const { email, firstName, lastName, phone, role, roles, companyName, designation, country, profilePicture } = req.body;

  if (!email || !firstName || !lastName || (!role && (!roles || roles.length === 0))) {
    return res.status(400).json({ success: false, message: 'Email, name, and role are required' });
  }

  const assignedRoles = roles && Array.isArray(roles) && roles.length > 0 ? roles : [role];
  const primaryRole = assignedRoles[0] || 'FARMER';
  ...
  // Default role-based avatar fallback if not uploaded
  const roleAvatarMap: Record<string, string> = {
    ADMIN: '/avatars/avatar_admin.jpg',
    ADMIN_MAKER: '/avatars/avatar_admin_maker.svg',
    FARMER: '/avatars/avatar_farmer.jpg',
    FARM_MANAGER: '/avatars/avatar_farm_manager.jpg',
    BUYER: '/avatars/avatar_buyer.svg',
    MANDI_AGENT: '/avatars/avatar_mandi_agent.svg',
    WORKER: '/avatars/avatar_worker.svg',
  };

  const finalProfilePic = profilePicture || roleAvatarMap[primaryRole] || undefined;
  ...
  const newUser = {
    ...
    roles: assignedRoles,
    companyName: companyName ? companyName.trim() : `${firstName}'s Enterprise`,
    country: country ? country.trim() : 'INDIA',
    designation: designation ? designation.trim() : primaryRole.replace('_', ' '),
    profilePicture: finalProfilePic,
    status: 'active' as const,
    recentActivityAt: 'Just now',
    permissions: [
      'farm_overview', 'plots_manage', 'crop_cycles', 'growth_stages', 'harvest_log',
      'task_schedule', 'labor_assign', 'proof_photos', 'expense_entry',
      'produce_listings', 'mandi_ticker', 'profile_view', 'profile_edit'
    ],
    internalUsers: [],
    createdAt: new Date().toISOString(),
  };
  ...
  phase1Store.auditLogs.unshift({
    ...
    entityId: newUser.id,
    oldValue: null,
    newValue: `Directly created ${primaryRole} user: ${newUser.email}`,
    timestamp: new Date().toISOString(),
  });

  return res.status(201).json({
    success: true,
    message: `Active ${primaryRole} user created successfully with default password AgriBridgeAI@2026.`,
    data: newUser,
  });
};

router.post('/auth/admin/create-user', handleAdminCreateUser);
router.post('/auth/admin-create-user', handleAdminCreateUser);

// Admin Bulk Create Users
router.post('/auth/admin/bulk-create-users', async (req: Request, res: Response) => {
  const authUser = getAuthUser(req);
...
router.put('/auth/users/:id/permissions', (req: Request, res: Response) => {
  const authUser = getAuthUser(req);
  const { id } = req.params;
...
router.put('/auth/profile', (req: Request, res: Response) => {
  ...
  if (!authUser && req.body.email) {
    authUser = phase1Store.users.find((u) => u.email.toLowerCase() === req.body.email.trim().toLowerCase()) || null;
  }
  if (!authUser) {
  ...
  if (firstName) authUser.firstName = firstName.trim();
  if (lastName) authUser.lastName = lastName.trim();
  if (companyName !== undefined) authUser.companyName = companyName.trim();
  if (phone !== undefined) authUser.phone = phone.trim();
  if (designation !== undefined) authUser.designation = designation.trim();
  if (country !== undefined) authUser.country = country.trim();
  if (profilePicture !== undefined) authUser.profilePicture = profilePicture;
  if (twoFactorEnabled !== undefined) authUser.twoFactorEnabled = Boolean(twoFactorEnabled);
  if (isEmailVerified !== undefined) authUser.isEmailVerified = Boolean(isEmailVerified);
...
router.post('/auth/send-email-otp', (req: Request, res: Response) => {
  const authUser = getAuthUser(req);
  if (!authUser) {
    return res.status(401).json({ success: false, message: 'Unauthorized' });
  }
...
router.post('/auth/verify-email', (req: Request, res: Response) => {
  ...
  if (email && email.trim()) authUser.email = email.trim().toLowerCase();
...
router.post('/auth/enable-2fa', (req: Request, res: Response) => {
  const authUser = getAuthUser(req);
  if (!authUser) {
    return res.status(401).json({ success: false, message: 'Unauthorized' });
  }
...
router.post('/auth/disable-2fa', (req: Request, res: Response) => {
  const authUser = getAuthUser(req);
  if (!authUser) {
    return res.status(401).json({ success: false, message: 'Unauthorized' });
  }
...
// Add Internal User
router.post('/auth/users/:id/internal-users', (req: Request, res: Response) => {
  const { id } = req.params;
  ...
  if (!target.internalUsers) target.internalUsers = [];
...
router.patch('/auth/users/:id/internal-users/:subUserId/status', (req: Request, res: Response) => {
...
router.get('/admin/live-prices', (req: Request, res: Response) => {
  const authUser = getAuthUser(req);
...
router.patch('/admin/live-prices/:id/block', (req: Request, res: Response) => {
  const authUser = getAuthUser(req);
  if (!authUser || (!authUser.roles.includes('ADMIN') && !authUser.roles.includes('SuperAdmin'))) {
...
router.post('/admin/live-prices/batch-block', (req: Request, res: Response) => {
  const authUser = getAuthUser(req);
  if (!authUser || (!authUser.roles.includes('ADMIN') && !authUser.roles.includes('SuperAdmin'))) {
    return res.status(403).json({ success: false, message: 'Forbidden: Admin privilege required' });
  }
...
router.post('/admin/live-prices/sync-gov', (req: Request, res: Response) => {
  const authUser = getAuthUser(req);
  if (!authUser || (!authUser.roles.includes('ADMIN') && !authUser.roles.includes('SuperAdmin'))) {
    return res.status(403).json({ success: false, message: 'Forbidden: Admin privilege required' });
  }
...
const handleGetPublicMandiPrices = (req: Request, res: Response) => {
  const authUser = getAuthUser(req);
...
const getAdminDashboardSummary = (_req: Request, res: Response) => {
