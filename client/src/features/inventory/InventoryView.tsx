        ...
        {items.map((item) => (
          ...
          >
            <div>
              ...
              {item.isLowStock && (
                <div className="mt-2.5 p-2 bg-amber-100/70 border border-amber-300 rounded-lg text-[11px] text-amber-900 font-semibold flex items-center gap-1.5">
                  <span>⚠️</span> Low Stock Alert: Below threshold ({item.lowStockThreshold} {item.unit})
                </div>
              )}
            </div>
            ...
            <form onSubmit={handleAddItem} className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <select value={category} onChange={(e) => setCategory(e.target.value)} className="w-full text-xs px-3 py-2 border rounded-lg">
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 space-y-4 shadow-xl">
            <div className="flex justify-between items-center border-b pb-3">
              ...
              <button onClick={() => setShowConsumeModal(null)} className="text-slate-400 hover:text-slate-600">✕</button>
            </div>
            <form onSubmit={handleConsume} className="space-y-3">
              <div className="p-3 bg-emerald-50 rounded-xl border border-emerald-200 text-xs text-emerald-900 space-y-1">
                <span className="font-bold block">✨ Auto-Expense Integration:</span>
              ...
              {consumeQty && (
                <div className="p-2 bg-slate-50 border rounded-lg text-xs flex justify-between">
